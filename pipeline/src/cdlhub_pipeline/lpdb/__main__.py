...
def run_load(dsn: str) -> None:
    ...
    if fix:
        print(
            ...
            f"cdl events placed {len(fix['cdl_events_placed'])}; "
            f"cdl duplicates deleted {len(fix['cdl_duplicates_deleted'])}; "
            f"mapless filled {len(fix['mapless_filled'])}; "
            f"mapless duplicates deleted {len(fix['mapless_duplicates_deleted'])}; "
            f"unheld backfilled {len(fix['unheld_backfilled'])}; "
